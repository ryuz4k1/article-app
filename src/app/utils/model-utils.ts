...
export function toUrl<S>(...propNames: PropsOf<S, string>[]): {[key: string]: (value: string) => {[key: string]: string} } {
  return propNames.reduce((acc, next) => ({...acc, [next]: (url: string) => ({[next]: environment.mediaUrl + url})}), {});
}
...
export function pickFrom<S, M extends {id: number} = {id: number}, T = S>(
  propName: PropsOf<S, number>,
  collection: M[],
  defaultValue: M = null,
  // tslint:disable-next-line:no-any
  modelPropName: PropsOf<T, M> = propName as any
): {[key: string]: (value: number) => {[key: string]: M} } {
  return {[propName]: (value: number) => ({[modelPropName]: collection.find(({id}) => id === value) || defaultValue})};
}
...
export function langualize<S>(prop: keyof S): {[key: string]: (value: string, current: {[key: string]: {[key: string]: string}}) => {[key: string]: {[key: string]: string}}} {
  return {
    ...
    // tslint:disable-next-line:max-line-length
    ...LANGUAGES.reduce((acc, next) => ({...acc, [prop + next]: (val: string, current: {[key: string]: {[key: string]: string}}) => ({[prop]: {...current[prop as string], [next]: val}})}), {}),
  };
}
